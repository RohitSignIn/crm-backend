...
const createTicket = async (data, userData) => {
  try {
    ...
    if (ticketResponse) {
      ...
      // };
      // sendNotification(
      //   sendNotificationEmailObject.subject,
      //   sendNotificationEmailObject.content,
      //   sendNotificationEmailObject.recepientEmails,
      //   sendNotificationEmailObject.requester,
      //   sendNotificationEmailObject.ticketId
      // );
...
const updateTicketById = async (ticketIdInfo, ticketInfo, currentUser) => {
  try {
    ...
    console.log("PREVIOUS", update.assignee);
    if (
      currentUser.userType != "admin" ||
      (currentUser.userType == "engineer" &&
        currentUser.email != update.assignedTo) ||
      (currentUser.userType == "customer" &&
        currentUser.email != update.createdBy)
    ) {
      return {
        error: "assignee is invalid",
      };
    }
    console.log(update.assignee, currentUser.email);
