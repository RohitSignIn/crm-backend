...
module.exports = function (app) {
  app.post(
    "/crmapp/api/v1/ticket/",
    authValidators.isUserAuthenticated,
    ticketController.createTicket
  );

  //get all tickets assigned to current user
  app.get(
    "/crmapp/api/v1/getMyAssignedTickets/",
    authValidators.isUserAuthenticated,
    ticketController.getMyAllAssignedTickets
  );

  //get all tickets created by current user
  app.get(
    "/crmapp/api/v1/getMyCreatedTickets/",
    authValidators.isUserAuthenticated,
    ticketController.getMyAllCreatedTickets
  );
  ...
  app.get(
    "/crmapp/api/v1/ticket/",
    authValidators.isUserAuthenticated,
    ticketController.getAllTicktes
  );

  //get all tickets by status //only admin and engineer can get all the tickets
  app.get(
    "/crmapp/api/v1/ticketbystatus/:status",
    authValidators.isUserAuthenticated,
    authValidators.isAdminOrEngineer,
    ticketController.getAllTicketsByStatus
  );

  //get all details of one ticket by id
  app.get(
    "/crmapp/api/v1/ticket/:id",
    authValidators.isUserAuthenticated,
